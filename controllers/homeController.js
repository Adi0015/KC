...
exports.getCopyright = async (req, res) => {
  res.status(200).render("copyright", { page: "copyright" });
};

exports.createContact = async (req, res) => {
  let { parent_name, parent_email, parent_phone, branch, message } = req.body;
...
exports.newImgCaptions = async (req, res) => {
  try {
    for (let i = 1; i <= 9; i++) {
      if (req.body[`label${i}`] !== "") {
        let cap = new imgCaption(i, req.body[`label${i}`]);
  ...
  try {
    for (let i = 1; i <= 9; i++) {
      if (req.body[`vdlabel${i}`] !== "") {
        let vdcap = new vdCaption(i, req.body[`vdlabel${i}`]);
